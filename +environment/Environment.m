    ...
    methods
        ...
        function interp_data = Interp(Env, data, headings, elevations, wavelengths)
            ...
            if any(isnan(interp_data))
                % Diagnostics
                disp('Environment axes:');
                disp(['Wavelength range: ', num2str([min(Env.wavelengths), max(Env.wavelengths)])]);
                disp(['Heading range: ', num2str([min(Env.headings), max(Env.headings)])]);
                disp(['Elevation range: ', num2str([min(Env.elevations), max(Env.elevations)])]);
