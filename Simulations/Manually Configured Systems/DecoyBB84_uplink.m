...
if small_sat
    Receiver_Telescope_Diameter=0.35;                                        % diameters in m
else
    Receiver_Telescope_Diameter=0.1;
end
OrbitDataFileLocation='500kmSSOrbitLLAT.txt';                              
Transmitter_Telescope_Diameter = 0.7;
...
QKDsystems = struct( ...
    ...
    'txDiam', {Transmitter_Telescope_Diameter, Transmitter_Telescope_Diameter, Transmitter_Telescope_Diameter},...  
    'rxDiam', {Receiver_Telescope_Diameter, Receiver_Telescope_Diameter, Receiver_Telescope_Diameter},...               
...
for i = 1:nQKDSystems    
    ...
    % Create satellite
    Sat{i} = createSatellite(Det{i}, OrbitDataFileLocation, QKDsystems(i).rxDiam, QKDsystems(i).Wavelength, QKDsystems(i).rxFOV, ideal_pass);
    
    % Create ground station
    GS{i} = createGroundStation(QKDsystems(i).txDiam,...
        QKDsystems(i).Wavelength, QKDsystems(i).rxFOV, Receiver_Jitter, ...
        Env, [55.909723,-3.319995,10], 'Heriot-Watt', Rep_Rate, MPNs, SPs, state_prep_error);
    ...
    % Run simulation
    Results{i} = nodes.QkdPassSimulation(Sat{i}, GS{i}, protocol.decoyBB84);
...
function Env = buildEnvironment(env_dir)
    ...
    Env.turbulence_model = environment.Turbulence_Model('Preset','2HV5-7');   % or 'HV5-7': sea level, '2HV5-7': bad day at sea level,  'HV10-10': typical astronomical , 'HV15-12' excellent site
end

% Satellite
function SimSat = createSatellite(Detector, OrbitDataFileLocation, RxDia, Wavelength, FOV, pass_type)    
    RxTelescope = components.Telescope(RxDia, ...
                                    'Wavelength', Wavelength, 'FOV', FOV);                              % transmitter telescope

    % using LLAT
    if pass_type
        SimSat = nodes.Satellite(RxTelescope, 'Detector', Detector, ...
        ...
        SimSat = nodes.Satellite(RxTelescope, ...
        'Detector', Detector, ...
...
% Ground Station
function SimGS = createGroundStation(TxDiameter, Wavelength, FOV, Jitter, Env, LLA, Name, RepetitionRate, MPNs, SPs, state_prep_error)
    TX_Src = components.Source(Wavelength, ...                                 % tx source
    ...
    TxTelescope = components.Telescope(TxDiameter, ... % 'FOV', FOV,...          % receiver telescope
        'Pointing_Jitter', Jitter, ...
        'Wavelength', Wavelength);
    SimGS = nodes.Ground_Station(TxTelescope,...                            % ground station
        'LLA', LLA, 'Name', Name, 'Source', TX_Src);
