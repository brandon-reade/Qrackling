...
rxSweep_m = [0.2 : 0.2 : 2, 3 : 1 : 10]';                                   % 0.2 steps from 0.2 to 2, then 1 steps from 2 to 10
...
if plot_best_vs_rxdiam
    ...
    % Best wavelength (min MeanQBER)
    plots.compare.plotBestWavelengthVsSweep(Tq, ...
        Metric="MeanQBER", ...
        GroupBy=["TxDiam_m"], ...
        BestMode="min", ...                    
        ShowBestWavelengthLabels=true, ...
...
if plot_heatmap
    ...
    plots.compare.plotSweepHeatmap(Tq, ...
    Metric="MeanQBER", ...
    XVar="WavelengthNm", ...
    YVar="RxDiam_m", ...
    GroupBy="TxDiam_m", ...
    FigureName="Heatmap: Mean QBER (candidate wavelengths)", ...
    ColorScale="linear", ...
    Interp="none");
end
